...
def parse_args():
    ...
    parser.add_argument('--min_freq', type=int, default=0, help='If > 0, use min_freq as the cutoff to create vocab.')
...
def load_tokens(filename):
    ''' Load tokens from file with filename.
        Args:
            filename (str): file path to load tokens.
        returns:
            tokens' list
    '''
    with open(filename) as fin:
        ...
        for d in data:
            tokens += d['token']
    print("{} tokens from {} examlpes loaded from {}.".format(len(tokens), len(data), filename))
    return tokens

def build_vocab(tokens, glove_vocab, min_freq):
    ''' Build the vocab from tokens and glove words. 
        Args:
            tokens (list): list of tokens occur in json file.
            glove_vocab (list): list of words in glove file.
            min_freq (int): cutoff freq.
        returns:
            vocab (list)
    '''
    counter = Counter(t for t in tokens) # build counter with tokens
    # if min_freq > 0, use min_freq to cutoff tokens, otherwise keep all words in glove words.
    if min_freq > 0:
